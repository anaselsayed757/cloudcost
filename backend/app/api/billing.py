...
@router.get("/history")
async def billing_history(db: AsyncSession = Depends(get_db)):
...
@router.get("/export")
async def export_billing_csv(db: AsyncSession = Depends(get_db)):
    """Export all cost records as CSV download."""
    result = await db.execute(
    ...
    for r in records:
        writer.writerow([
            ...
            r.cpu_cost, r.ram_cost,
            r.network_cost, r.total_cost
        ])
...
@router.post("/receipt")
async def send_client_receipt(body: ReceiptRequest, db: AsyncSession = Depends(get_db)):
    from app.email_service import send_email

    if "@" not in body.client_email:
        raise HTTPException(status_code=400, detail="Valid client email is required")

    recipient_email = body.recipient_email or body.client_email
    if "@" not in recipient_email:
        raise HTTPException(status_code=400, detail="Valid recipient email is required")

    vm_result = await db.execute(select(VM.instance).where(VM.owner_email == body.client_email))
...
async def update_tariff(tariff_id: str, body: TariffUpdate,
